...
一部の過去の開発記録や履歴レポートには、実行・例外記録・レポート生成を行った当時の絶対ローカルパスの構造が残っている。初回公開前のプライバシー対応として、そのうち正確に14ファイルに含まれていたローカルOSのユーザー名だけを`<local-user>`へ置換した。周囲のパス、過去の結果値、科学的内容はそれ以外変更していない。元のGitオブジェクトと公開版Gitオブジェクトの対応は、匿名化したユーザー名を復元できない形で[`../../canonical/canonical-v1/privacy-redaction-provenance.json`](../../canonical/canonical-v1/privacy-redaction-provenance.json)に記録している。これらのパスは現行コードの依存先や導入要件ではなく、現在の公開文書と実行コマンドはリポジトリ相対または移植可能なパスを使用する。
