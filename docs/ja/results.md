...
この比較には、Xへ投稿した動画、README GIFの生成に使った高解像度の元動画、GitHub Releaseに添付したMP4という3つの別ファイルがあります。いずれもv240/v966の同じ過去比較を示しますが、同一の描画・バイナリではありません。GitではREADME GIFと3者の役割・ハッシュ値を記録したmanifestだけを追跡し、MP4本体と生の再生記録はGitの外に置きます。詳細は [`../../release/release-assets-v0.1.0.json`](../../release/release-assets-v0.1.0.json) を参照してください。
