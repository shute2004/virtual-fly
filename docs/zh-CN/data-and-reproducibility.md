...
供第三方下载的大型检查点与 GitHub 源码仓库分开发布，以 Hugging Face 的 `shute2004/virtual-fly` 作为统一入口。
...
少量历史开发记录和历史报告仍保留原始运行、异常记录或报告生成时使用的绝对本地路径。为了保持原始来源记录，这些字符串不会在事后改写。它们不是当前代码的依赖，也不是安装要求；当前公开文档和执行命令使用仓库相对路径或可移植路径。
