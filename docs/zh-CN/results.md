...
2026 年 9 月 19 日，我们使用 `canonical/canonical-v1/reproduce.sh`，从原始实验 commit `7fa464a` 创建独立工作区，并从头到尾执行了完整复现流程。这次验证发生在公开前的隐私处理之前，不会被描述成是在 `f9c86c9` 上执行。由于公开等价提交没有改变可执行代码、配置或科学产物生成逻辑，因此没有仅因隐私脱敏而重新运行完整训练。
