...
use crate::{ConnectomeSnapshot, NeuralParams, NeuralState, PlasticFastGraph, Stimulus};
...
impl GpuPopulationRuntime {
    ...
    async fn new_async(
        ...
        let total_neurons = n.checked_mul(slot_count).context("population neuron-state size overflow")?;
        let total_plastic = p.checked_mul(slot_count).context("population plastic-state size overflow")?;
        ...
        let count_start = pre_start.checked_add(m).context("population topology offset overflow")?;
        let meta_start = count_start.checked_add(m).context("population topology offset overflow")?;
        let plastic_row_start = meta_start.checked_add(n).context("population topology offset overflow")?;
        let plastic_edge_start = plastic_row_start
            .checked_add(plastic.row_offsets.len())
            .context("population topology offset overflow")?;
        let plastic_post_start = plastic_edge_start.checked_add(p).context("population topology offset overflow")?;
        let topology_len = plastic_post_start.checked_add(p).context("population topology size overflow")?;
        ...
        let (device, queue) = adapter
            .request_device(&wgpu::DeviceDescriptor {
                label: Some("virtual-fly sparse population neural compute device"),
            ...
            .await
            .context("failed to create sparse population GPU device")?;
        ...
        topology.extend((0..snapshot.neuron_count()).map(|neuron| snapshot.packed_neuron_metadata(neuron)));
        ...
        let plastic_synapse_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("vf sparse population plastic synapse state"),
            size: byte_size::<SynapseStateGpu>(total_plastic),
            usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        let transaction_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("vf sparse population plastic transaction"),
            size: byte_size::<TransactionStateGpu>(total_plastic),
            usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        let zero_spikes = vec![0u32; total_neurons];
        let spikes_a = create_storage_init(&device, "vf sparse population spikes a", bytemuck::cast_slice(&zero_spikes), true);
        let spikes_b = create_storage_init(&device, "vf sparse population spikes b", bytemuck::cast_slice(&zero_spikes), true);
        let external_cpu = vec![0.0f32; total_neurons];
        let external_buffer = create_storage_init(
            &device,
            "vf sparse population external current",
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("vf sparse population pipeline layout"),
            bind_group_layouts: &[Some(&layout)],
            immediate_size: 0,
        });
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("vf sparse population neural shader"),
            source: wgpu::ShaderSource::Wgsl(Cow::Borrowed(include_str!("population_sparse.wgsl"))),
        });
        ...
        let bind_group_ab = create_population_bind_group(
            &device, &layout, &topology_buffer, &neuron_state_buffer, &spikes_a, &spikes_b,
            &external_buffer, &plastic_synapse_buffer, &transaction_buffer, &global_weight_buffer,
            &params_buffer, &control_buffer, "vf sparse population a->b",
        );
        let bind_group_ba = create_population_bind_group(
            &device, &layout, &topology_buffer, &neuron_state_buffer, &spikes_b, &spikes_a,
            &external_buffer, &plastic_synapse_buffer, &transaction_buffer, &global_weight_buffer,
            &params_buffer, &control_buffer, "vf sparse population b->a",
        );

        let spike_gather_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("vf sparse population spike gather layout"),
            entries: &[storage_layout(0, true), storage_layout(1, true), storage_layout(2, false)],
        });
        let spike_gather_pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("vf sparse population spike gather pipeline layout"),
        ...
        let spike_gather_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("vf sparse population spike gather shader"),
        ...
        let spike_gather_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("gather_sparse_population_spikes"),
    ...
    pub fn plastic_edge_count(&self) -> usize { self.plastic_edge_count }

    pub fn load_global_weights(&mut self, weights: &[f32]) -> Result<()> {
        if weights.len() != self.edge_count {
            bail!("global weight count {} does not match edge count {}", weights.len(), self.edge_count);
        }
        self.queue.write_buffer(&self.global_weight_buffer, 0, bytemuck::cast_slice(weights));
    ...
    pub fn step_batch_with_read(
        ...
        let plastic_dispatch = if plasticity && self.plastic_edge_count > 0 {
            Some(dispatch_grid((self.slot_count * self.plastic_edge_count) as u32, max_groups)?)
        } else { None };
        ...
        let mut encoder = self.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("vf sparse population neural+gather encoder"),
        });
        self.encode_neural_step(&mut encoder, bind_group, neuron_dispatch, plastic_dispatch, plasticity);
        ...
        let index_buffer = device_storage_init(&self.device, "vf sparse population gather indices", bytemuck::cast_slice(&indices));
        let output_buffer = self.device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("vf sparse population gather output"),
            size: read_bytes,
            usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC,
            mapped_at_creation: false,
        });
        let staging = self.device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("vf sparse population gather staging"),
            size: read_bytes,
            usage: wgpu::BufferUsages::MAP_READ | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        let gather_bind_group = self.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("vf sparse population gather bind group"),
        ...
        let gather_dispatch = dispatch_grid(indices.len() as u32, max_groups)?;
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("vf sparse population motor gather"),
    ...
    pub fn step_batch_no_read(
        ...
        let mut encoder = self.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("vf sparse population repeated neural encoder"),
        });
    ...
    pub fn commit_and_restart_slot(&mut self, slot: usize) -> Result<()> {
        ...
        let mut encoder = self.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("vf sparse population commit+restart encoder"),
        });
        let bind_group = if self.current_is_b { &self.bind_group_ba } else { &self.bind_group_ab };
        if self.plastic_edge_count > 0 {
            {
                let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                    label: Some("vf sparse population commit slot transaction"), timestamp_writes: None,
                });
            ...
            {
                let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                    label: Some("vf sparse population restart slot synapses"), timestamp_writes: None,
                });
                pass.set_bind_group(0, bind_group, &[]);
    ...
    pub fn restart_slot(&mut self, slot: usize) -> Result<()> {
        ...
        let neuron_dispatch = dispatch_grid(self.neuron_count as u32, max_groups)?;
        let mut encoder = self.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("vf sparse population restart slot encoder"),
        });
        let bind_group = if self.current_is_b { &self.bind_group_ba } else { &self.bind_group_ab };
        if self.plastic_edge_count > 0 {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
    ...
    fn restart_all_slots(&mut self) -> Result<()> {
        ...
        for slot in 0..self.slot_count { self.restart_slot(slot)?; }
        Ok(())
    }
    ...
    fn encode_neural_step(
        ...
        let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("vf sparse population neural step"), timestamp_writes: None,
        });
        pass.set_bind_group(0, bind_group, &[]);
    ...
    fn encode_reset_slot_neurons(&self, encoder: &mut wgpu::CommandEncoder, neuron_dispatch: (u32, u32)) {
        for bind_group in [&self.bind_group_ab, &self.bind_group_ba] {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("vf sparse population reset slot neurons"), timestamp_writes: None,
            });
            pass.set_bind_group(0, bind_group, &[]);
    ...
    fn prepare_step(&mut self, stimuli_by_slot: &[Vec<Stimulus>], active_slots: &[bool]) -> Result<u32> {
        if stimuli_by_slot.len() != self.slot_count || active_slots.len() != self.slot_count {
            bail!("population step requires exactly {} slot payloads", self.slot_count);
        }
        self.external_cpu.fill(0.0);
        let mut active_mask = 0u32;
        for slot in 0..self.slot_count {
            if !active_slots[slot] { continue; }
        ...
        self.queue.write_buffer(&self.external_buffer, 0, bytemuck::cast_slice(&self.external_cpu));
        Ok(active_mask)
    }

    fn validate_slot(&self, slot: usize) -> Result<()> {
        if slot >= self.slot_count {
            bail!("population slot {slot} is out of range 0..{}", self.slot_count);
        }
        Ok(())
    }
    ...
    fn map_staging<T: Pod + Copy>(&self, staging: &wgpu::Buffer) -> Result<Vec<T>> {
        ...
        for _ in 0..120 {
            ...
            match rx.try_recv() {
                Ok(result) => { result.context("population GPU readback map failed")?; mapped = true; break; }
                Err(mpsc::TryRecvError::Empty) => continue,
                Err(mpsc::TryRecvError::Disconnected) => bail!("population GPU readback callback channel disconnected"),
            }
        }
        if !mapped { bail!("population GPU readback timed out"); }
        let view = slice.get_mapped_range().context("failed to access population mapped GPU readback range")?;
    ...
    fn read_buffer<T: Pod + Copy>(&self, source: &wgpu::Buffer, count: usize) -> Result<Vec<T>> {
        ...
        let staging = self.device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("vf sparse population readback staging"),
            size,
            usage: wgpu::BufferUsages::MAP_READ | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        let mut encoder = self.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("vf sparse population readback encoder"),
        });
...
fn byte_size<T>(count: usize) -> u64 { (count.max(1) * std::mem::size_of::<T>()) as u64 }
fn exact_byte_size<T>(count: usize) -> u64 { (count * std::mem::size_of::<T>()) as u64 }

fn dispatch_grid(item_count: u32, max_groups_per_dimension: u32) -> Result<(u32, u32)> {
    if item_count == 0 { return Ok((0, 0)); }
    if max_groups_per_dimension == 0 { bail!("GPU reports zero max compute workgroups per dimension"); }
    ...
    if y > max_groups_per_dimension {
        bail!("population GPU dispatch requires {total_groups} workgroups, exceeding a {max_groups_per_dimension}x{max_groups_per_dimension} grid");
    }
    Ok((x, y))
}
...
fn create_storage_init(device: &wgpu::Device, label: &'static str, contents: &[u8], cpu_writable: bool) -> wgpu::Buffer {
    let mut usage = wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC;
    if cpu_writable { usage |= wgpu::BufferUsages::COPY_DST; }
    device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: Some(label), contents, usage })
}

fn device_storage_init(device: &wgpu::Device, label: &'static str, contents: &[u8]) -> wgpu::Buffer {
...
fn create_population_bind_group(
    ...
    device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        entries: &[
            ...
            wgpu::BindGroupEntry { binding: 1, resource: neurons.as_entire_binding() },
            ...
            wgpu::BindGroupEntry { binding: 4, resource: external.as_entire_binding() },
            wgpu::BindGroupEntry { binding: 5, resource: plastic_synapses.as_entire_binding() },
            wgpu::BindGroupEntry { binding: 6, resource: transactions.as_entire_binding() },
            wgpu::BindGroupEntry { binding: 7, resource: global_weights.as_entire_binding() },
            wgpu::BindGroupEntry { binding: 8, resource: params.as_entire_binding() },
            wgpu::BindGroupEntry { binding: 9, resource: control.as_entire_binding() },
        ],
    })
}
...
mod tests {
    ...
    #[test]
    fn sparse_population_dispatch_tiles_plastic_edge_sets() {
