"""Machine-readable run-history schema and summary aggregation."""
...
def merge_history_rows(existing: list[dict[str, str]], row: dict[str, Any]) -> list[dict[str, str]]:
    """Replace one semantic run identity while preserving historical rows."""
    rows = [item for item in existing if item.get("run_key") != row["run_key"]]
