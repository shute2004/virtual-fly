...
def _body_config(config: dict[str, Any]):
    from virtual_fly.embodiment.config import FlyppyBodyConfig

    return FlyppyBodyConfig.from_mapping(config)


def _capture_physics_trace(config: dict[str, Any]) -> bool:
...
def _worker_main(
    ...
    try:
        ...
        from virtual_fly.embodiment.direct_vision_bodyexclude import (
        ...
        capture_physics_trace = _capture_physics_trace(config)
        slots = {
                    ...
                    reset_flyppy_stack(
                        ...
                        gate_center_overrides={
                            ...
                            for key, value in dict(
                                reset_payload.get("gate_center_overrides") or {}
                            ).items()
                        },
                    )
                    replies[slot_id] = {
            ...
            if command == "act":
                ...
                for raw_id, active_values in dict(payload).items():
                    ...
                    physics_trace: list[dict[str, object]] | None = (
                        [] if capture_physics_trace else None
                    )
