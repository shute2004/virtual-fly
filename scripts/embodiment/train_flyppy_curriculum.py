...
start near the first gate's vertical center and at the high end of the normal
flight-speed range. Every successful first-gate passage moves the initial
condition toward the real target (z=5 mm, vx=300 mm/s). Once the target is
reached, subsequent episodes are ordinary target-condition training.
...
def load_state(path: Path, *, default_z: float, default_speed: float) -> dict[str, object]:
    if not path.exists():
        return {
            "schema_version": 1,
    ...
    if int(state.get("schema_version", 0)) != 1:
        raise RuntimeError(f"unsupported curriculum state schema in {path}")
    return state
...
def main() -> int:
    ...
    if any(not math.isfinite(value) or value <= 0.0 for value in numeric_positive):
        raise SystemExit("curriculum speed/step parameters must be finite and positive")
    if not math.isfinite(args.curriculum_target_z_mm):
        raise SystemExit("curriculum-target-z-mm must be finite")
    ...
    state = load_state(state_path, default_z=default_z, default_speed=default_speed)
    if not checkpoint.joinpath("manifest.json").exists() and not args.fresh:
        # A curriculum-state file without a usable learned checkpoint cannot be
        # continued coherently; restart only the curriculum metadata.
        state = load_state(Path("/__missing_curriculum_state__"), default_z=default_z, default_speed=default_speed)

    print(
        "curriculum first_gate_x={:.3f} gate_center_z={:.3f} target_z={:.3f} "
        "target_speed={:.1f}".format(
    ...
    for local_episode in range(args.episodes):
        ...
        print(
            "curriculum_episode={} spawn_z={:.3f} initial_vx={:.1f} resume={}".format(
                int(state["curriculum_episodes"]), spawn_z, speed, resume
            )
        )
        ...
        state["curriculum_complete"] = bool(
            math.isclose(float(state["spawn_z_mm"]), args.curriculum_target_z_mm, abs_tol=1e-9)
            and math.isclose(
        ...
        print(
            "curriculum_result passed={} max_x={:.3f} next_spawn_z={:.3f} "
