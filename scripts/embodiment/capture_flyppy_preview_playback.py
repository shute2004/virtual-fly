...
from virtual_fly.reproducibility import HALTERE_FULL_KIND, HALTERE_TIMING_KIND, git_provenance, dependency_lock_provenance, run_semantics, validate_haltere_map, validate_production_snapshot
...
def parse_args():
    ...
    p.add_argument('--haltere-sensory-map',type=Path,default=Path('artifacts/malecns-v1.0/haltere-campaniform-sensory-v1.json')); p.add_argument('--haltere-sensory-kind',choices=(HALTERE_FULL_KIND,HALTERE_TIMING_KIND),default=HALTERE_FULL_KIND); p.add_argument('--haltere-current-gain',type=float,default=0.0); p.add_argument('--haltere-transduction',choices=('angular-acceleration-v1','interaction-load-v2'),default='angular-acceleration-v1')
    p.add_argument('--environment-version',choices=('v3','v4','v5','v6','v7'),required=True); p.add_argument('--flight-body-version',choices=('v3','v4','v5','v6','v7','v8'),required=True); p.add_argument('--vertical-steering-gain',type=float,default=1.0); p.add_argument('--measured-steering-gain',type=float,default=1.0); p.add_argument('--neutral-trim-strength',type=float,default=1.0); p.add_argument('--steering-tau-ms',type=float,default=12.0); p.add_argument('--steering-spike-increment',type=float,default=0.85); p.add_argument('--course-seed',type=int,default=DEFAULT_COURSE_SEED); p.add_argument('--gate-count',type=int,default=6)
...
def main():
    ...
    snapshot_meta=validate_production_snapshot(snapshot); haltere_meta=validate_haltere_map(haltere_map,a.haltere_sensory_kind,snapshot=snapshot)
    ...
    config={'snapshot':str(snapshot),'seed':a.course_seed,'gate_count':a.gate_count,'environment_version':a.environment_version,'flight_body_version':a.flight_body_version,'vertical_steering_gain':a.vertical_steering_gain,'measured_steering_gain':a.measured_steering_gain,'neutral_trim_strength':a.neutral_trim_strength,'steering_tau_ms':a.steering_tau_ms,'steering_spike_increment':a.steering_spike_increment,'wing_motor_map':str(snapshot/'wing-motor-neurons-v0.json'),'body_motor_map':str(snapshot/'body-motor-neurons-v0.json'),'retinotopic_map':str(snapshot/'retinotopic-vision-v1.json'),'haltere_sensory_map':str(haltere_map),'haltere_sensory_kind':a.haltere_sensory_kind,'photoreceptor_current_gain':a.photoreceptor_current_gain,'haltere_current_gain':a.haltere_current_gain,'haltere_transduction':a.haltere_transduction,'capture_physics_trace':True}
    ...
    try:
        ...
        with PopulationNeuralBridgeClient(snapshot=snapshot,groups=snapshot/'embodiment-groups-v0.json',slots=1) as brain:
            ...
            payload={'schema_version':1,'kind':'flyppy-offline-preview-playback','brain_state':'fresh-snapshot' if a.fresh_brain else 'learned-checkpoint','source_experiment':None if a.fresh_brain else str(production),'checkpoint_neural_step':int(loaded.get('step',0 if a.fresh_brain else -1)),'global_weight_version':int(brain.global_weight_version),'environment_version':a.environment_version,'flight_body_version':a.flight_body_version,'vertical_steering_gain':a.vertical_steering_gain,'neutral_trim_strength':a.neutral_trim_strength,'steering_tau_ms':a.steering_tau_ms,'steering_spike_increment':a.steering_spike_increment,'haltere_current_gain':a.haltere_current_gain,'haltere_sensory_kind':a.haltere_sensory_kind,'haltere_sensory_map_sha256':haltere_meta['sha256'],'haltere_transduction':a.haltere_transduction,'vision_runtime':os.environ.get('VF_FLYPPY_VISION_MODE','direct-ray'),'vision_rays_per_ommatidium':int(os.environ.get('VF_FLYPPY_OMMATIDIA_RAYS','13')),'snapshot_semantics':snapshot_meta,'code':git_provenance(ROOT),'dependency_lock':dependency_lock_provenance(ROOT),'semantics':run_semantics(),'course_seed':a.course_seed,'gate_count':a.gate_count,'gate2_center_z_mm':a.gate2_center_z_mm,'spawn_x_mm':a.spawn_x_mm,'spawn_z_mm':a.spawn_z_mm,'initial_speed_mm_s':a.initial_speed_mm_s,'initial_vz_mm_s':a.initial_vz_mm_s,'physics_steps_per_control':a.physics_steps,'physics_substep_stride':a.physics_substep_stride,'playback_fps':a.playback_fps,'passed_gates':passed,'terminal_reason':terminal_reason,'control_steps':control,'plasticity':bool(a.plasticity),'reward_from_gate':int(a.reward_from_gate),'frame_count':len(frames),'frames':frames}
